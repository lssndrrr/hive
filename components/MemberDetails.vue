        ...
        <div class="flex-grow overflow-y-auto px-8 py-4 space-y-6">
            <div class="flex flex-col items-center justify-center space-y-4 text-center">
                <UAvatar
                    ...
                    :ui="{ background: 'bg-[#A86523]', icon: { base: 'text-[#FCEFCB]' } }"
                />
                <h3 class="text-xl font-bold text-[#A86523]">{{ member.name || 'Member Name' }}</h3>
            </div>
            ...
            <div class="space-y-1">
                ...
                <p class="text-sm text-[#A86523]">
                    {{ member.joinedDate ? formatDate(member.joinedDate) : 'Date not available' }}
                </p>
            </div>
